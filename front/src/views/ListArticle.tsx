          ...
          {articles.map((article:any) => (
            <div key={article.id} className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="mt-4 flex justify-between text-sm text-gray-600">
                <button className="text-blue-500 hover:underline">View Comments ({article.comments.length})</button>
                <button className="text-blue-500 hover:underline">View Likes ({article.likes.length})</button>
              </div>
            </div>
          ))}
        </div>
